	...
	return (function loop(rts) {
		return rts.map(item => {
			...
			const route = {
				...
				component: Vue.component(`docs-layout-parent`, function (
					resolve,
					reject,
				) {
					resolve({
            ...
            `,
						components: {
							MardownDoc () {
								if(item.path === "\/statement"){
									return import(`../../components/index.md`);
								}
								return import(`../../components${routePath}/index.md`);
							}
						},
					});
				}),
	...
	const routes = Object.keys(config).map(key => {
		...
		if (nav.groups) {
			nav.groups.forEach(group => {
				...
				if (list) {
					route.children = [...route.children, ...loopPath(list)];
				}
			});
		}
